...
export default function ReembolsoPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        solicitante: '',
        data_solicitacao: '', // MANTIDO: Valor inicial vazio para SSR
        beneficiado: '',
        departamento: '',
        cpf_cnpj: '',
        banco: '',
    ...
    // FIX HYDRATION: Executa apenas no cliente e define o estado como montado
    useEffect(() => {
        setIsClient(true);
    ...
    const handleSubmit = async (e) => {
        ...
        // Ponto 1: Validação condicional do Artigo CLT
        const isPagamento = formData.tipo_solicitacao === 'pagamento';
        if (isPagamento && !formData.concordo) {
            setStatus({ submitting: false, error: 'Você precisa concordar com os termos da CLT para SOLICITAÇÃO DE PAGAMENTO.', success: false });
            return;
        }

        // Ponto 2: Validação do E-mail do Gestor
        if (!formData.email_gestor.trim()) {
            setStatus({ submitting: false, error: 'O campo de E-mail do Gestor é obrigatório.', success: false });
            return;
        }
        ...
        try {
            ...
            setFormData(prev => ({ ...prev, concordo: false, email_gestor: '' })); // Limpa campo gestor
    ...
    return (
        <div className="min-h-screen bg-gray-200 dark:bg-gray-900 p-2 md:p-4 font-sans transition-colors duration-200">
            <Head><title>Requisição Financeira - Maglog</title></Head>
            ...
            <div className="w-full max-w-[98%] md:max-w-[95%] mx-auto bg-white dark:bg-gray-800 p-4 md:p-8 rounded-lg shadow-xl border-t-8 border-cyan-900 dark:border-cyan-600">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* FIX HYDRATION: O valor só é exibido se for lado do cliente, mas mantemos o input para o layout */}
                        <div><label className={labelStyles}>Data</label><input disabled value={isClient ? formData.data_solicitacao : ''} className={`${inputStyles} bg-gray-200 dark:bg-gray-600 text-gray-500 dark:text-gray-300`} /></div>
                    ...
                    {/* Ponto 2: Campo E-mail do Gestor */}
                    <div>
                        <label className={labelStyles}>Enviar E-mail de Cópia ao Gestor (Obrigatório)</label>
                        <input
                            type="email"
                            required
                            name="email_gestor"
                    ...
                    {/* Ponto 1: Condicional do Artigo CLT */}
                    {formData.tipo_solicitacao === 'pagamento' && (
